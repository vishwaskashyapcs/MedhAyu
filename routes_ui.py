...
# #    if (r === "qa") {
# #     // Ask the server if this actor can see QA (must be in Quality Control)
# #     const uid = actorSelect.value;
# #     try {
# #       const resp = await fetch(`/auth/can_see_qa/${uid}`);
# #       if (resp.ok) {
# #         superSection.classList.remove("hidden");
# #       } else {
# #         roleSelect.value = "user";
# #         userSection.classList.remove("hidden");
# #         alert("Access denied: only Quality Control can view the Super Admin (QA) board.");
# #       }
# #     } catch {
# #       roleSelect.value = "user";
# #       userSection.classList.remove("hidden");
# #       alert("Could not verify permissions. Showing user view.");
# #     }
# #   }
# # }
# # }
...
# async function loadQA() {
#   const uid = actorSelect.value;
#   const r = await fetch(`/board/qa?user_id=${uid}`);
#   if (!r.ok) {
#     superList.innerHTML = `<div class="text-sm text-red-600">Access denied.</div>`;
#     return;
#   }
#   const data = await r.json();
#   superList.innerHTML = "";
...
#       const status = row.querySelector(".status").value;
#       const id = btn.getAttribute("data-id");
#       const r = await fetch(`/cr/${id}/override?user_id=${uid}`, {
#         method:"POST", headers:{"Content-Type":"application/json"},
#         body: JSON.stringify({ department: dept, status })
#       });
#       if (!r.ok) alert("override failed");
...
function showToast(message, variant="info", timeoutMs=3600) {
  const base = "rounded-lg shadow-lg px-4 py-3 text-sm flex items-start gap-2 border";
  const styles = {
  ...
  wrap.className = `${base} ${styles[variant]||styles.info} fade`;
  wrap.innerHTML = `
    <div class="mt-0.5">${message}</div>
    <button class="ml-2 text-xs underline opacity-70 hover:opacity-100">dismiss</button>
  `;
  wrap.querySelector("button").onclick = () => toastHost.removeChild(wrap);
  toastHost.appendChild(wrap);
  if (timeoutMs > 0) setTimeout(() => { wrap.isConnected && toastHost.removeChild(wrap); }, timeoutMs);
}
...
async function renderImpact(crId) {
  ...
  try {
    ...
    if (data.error) { box.innerHTML = "Error loading impact"; return; }
    const sla = data.sla_target_h ?? "—";
    box.innerHTML = `<div><b>🎯 Estimated Processing Time:</b> ${sla}h</div>`;
  } catch (err) {
    box.innerHTML = "⚠️ Impact data unavailable.";
  }
}
...
async function setRoleUI() {
  ...
  if (r === "dept_head") {
    adminSection.classList.remove("hidden");
    // Auto-load inbox on open so notifications can fire
    loadInbox(true);
    return;
  }
  ...
  if (!recording) {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        micStatus.textContent = "Uploading audio…";
        const resp = await fetch("/voice/stt", { method:"POST", body: form });
        if (!resp.ok) { micStatus.textContent = "Server STT failed."; return; }
        const data = await resp.json();
        const t = (data.text || "").trim();
        if (t) {
          const low = t.toLowerCase();
...
btnSubmit.addEventListener("click", async () => {
  ...
  // Toast: submitted
  showToast(`✅ CR #${currentCR} submitted. Click “Run AI” to route.`, "ok");
});
...
btnRunAI.addEventListener("click", async () => {
  ...
  // Toast: routed (or manual review fallback)
  if (data.predicted_department && data.predicted_department !== "Manual Review" && data.owner_name) {
    showToast(`📦 CR #${currentCR} routed to <b>${data.predicted_department}</b> — owner: <b>${data.owner_name}</b>.`, "ok");
  } else {
    showToast(`🕵️ CR #${currentCR} submitted for manual review (low confidence).`, "warn");
  }
});

/* ---------------- Dept Head Inbox ---------------- */
btnRefreshInbox.addEventListener("click", () => loadInbox(false));

// Remember last seen CR IDs per dept-head user to avoid duplicate popups
const lastInboxIdsByUser = Object.create(null);

/**
 * @param {boolean} initialOpen - when opening the inbox UI for the first time in this session
 */
async function loadInbox(initialOpen=false) {
  const uid = actorSelect.value || 2; // default to Manufacturing head
  const r = await fetch(`/inbox/dept_head/${uid}`);
  const data = await r.json();
  adminList.innerHTML = "";
  ...
  // Detect new items for toast (skip on first open to prevent spam)
  const prev = new Set(lastInboxIdsByUser[uid] || []);
...
`;
    ...
    if (!initialOpen && !prev.has(cr.id)) {
      showToast(`📨 Received new CR #${cr.id} — status: <b>${cr.status}</b>.`, "info");
    }
  });

  // Save snapshot
  lastInboxIdsByUser[uid] = Array.from(current);
  ...
  adminList.querySelectorAll(".approve").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      const r = await fetch(`/cr/${id}/approve`, { method:"POST" });
      if (r.ok) {
        showToast(`👍 Approved CR #${id}. Moved to QA_REVIEW.`, "ok");
        loadInbox(false);
      } else {
        showToast(`⚠️ Approve failed for CR #${id}.`, "error");
      }
    });
  });
  ...
  adminList.querySelectorAll(".reroute").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      if (r.ok) {
        showToast(`🔁 Rerouted CR #${id} to <b>${to}</b>.`, "ok");
        loadInbox(false);
      } else {
        showToast(`⚠️ Reroute failed for CR #${id}.`, "error");
      }
    });
  });
}
...
`;
  ...
  superList.querySelectorAll(".apply").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      const status = row.querySelector(".status").value;
      const id = btn.getAttribute("data-id");
      const r = await fetch(`/cr/${id}/override?user_id=${actorSelect.value}`, {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ department: dept, status })
      });
      if (!r.ok) showToast(`⚠️ Override failed for CR #${id}.`, "error");
      else {
        showToast(`🛠️ Overrode CR #${id}: dept=${dept || "—"}, status=${status}.`, "ok");
        loadQA();
      }
    });
  });
