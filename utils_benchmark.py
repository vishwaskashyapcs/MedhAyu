...
#     if not a or not b:
#         return None
#     # make tz-aware if needed (assume UTC)
#     if a.tzinfo is None: a = a.replace(tzinfo=timezone.utc)
#     if b.tzinfo is None: b = b.replace(tzinfo=timezone.utc)
#     return max(0.0, (b - a).total_seconds() / 3600.0)

# def estimate_manual_baseline_for(cr: ChangeRequest) -> Tuple[Optional[float], str]:
#     """
#     Returns (hours, source). Source is one of:
...
#     - "unknown"
#     Priority is: CR override -> dept median (manual field) -> dept cycle time
#                  -> global median (manual field) -> global cycle time -> unknown
#     """
...
#     # 2) department-scoped medians using real cycle time from status history (NEW→QA_REVIEW/IMPLEMENTED)
#     #    This approximates "how long manual used to take", based on your history.
#     #    We measure from the first NEW to the first of QA_REVIEW|IMPLEMENTED|CLOSED.
#     crs_dept = db.session.query(ChangeRequest).filter(ChangeRequest.department == cr.department).all()
#     durations = []
#     for r in crs_dept:
#         # find first NEW and first terminal-ish step
...
#     # 4) global median of cycle time (across all departments)
